...
bool InputManager::UpdateKeyState()
{
	...
	// Alt+Enterを除外
	if((keyState_[0x38] || keyState_[0xB8]) && keyState_[0x1C]){
		keyState_[0x38] = 0;
		keyState_[0xB8] = 0;
		keyState_[0x1C] = 0;
	}
...
bool InputManager::UpdateMouseState()
{
	...
	mouseState_.prePosX = mouseState_.posX;
	mouseState_.prePosY = mouseState_.posY;
	...
	mouseState_.differenceX = mouseState_.posX - mouseState_.prePosX;
	mouseState_.differenceY = mouseState_.posY - mouseState_.prePosY;
	...
	// ボタンが押されていたらフレーム数を加算
	for(int i = MouseInput::BUTTON_MAX - 1; i >= 0; --i){
...
bool InputManager::SetVibration(DWORD joystickID, float leftSpeed, float rightSpeed)
{
	...
	vibration.wLeftMotorSpeed = (WORD)(65535 * Clamp(leftSpeed, 0.0f, 1.0f));
	vibration.wRightMotorSpeed = (WORD)(65535 * Clamp(rightSpeed, 0.0f, 1.0f));
...
// ジョイスティックの入力情報の取得
//------------------------------------------------------------------------------
const Joystick& InputManager::GetJoystick(DWORD JoystickID)
{
