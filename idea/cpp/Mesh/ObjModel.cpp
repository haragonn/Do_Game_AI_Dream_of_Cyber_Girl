...
bool ObjModel::LoadObjMeshFromFile(const char * pFileName)
{
	...
	if(!file.is_open()){
		SetDebugMessage("ObjMeshLoadError! [%s] をファイルから読み込めませんでした。\n", pFileName);
		return false;
	}
...
bool ObjModel::LoadObjMeshFromArchiveFile(const char * pArchiveFileName, const char * pFileName)
{
	...
	if(!loader.Load(pArchiveFileName, pFileName)){
		SetDebugMessage("ObjMeshLoadError! [%s] を [%s] から読み込めませんでした。\n", pFileName, pArchiveFileName);
		return false;
	}
...
bool ObjModel::LoadObjMeshFromStorage(const char * pFileName)
{
	...
	// ストレージから読み込む
	// 頂点
	vertexSize_ = StorageManager::Instance().GetObjModel(pFileName).vertexSize_;
	pVertexBuffer_ = StorageManager::Instance().GetObjModel(pFileName).pVertexBuffer_;
	copy(StorageManager::Instance().GetObjModel(pFileName).vecVertex_.begin(), StorageManager::Instance().GetObjModel(pFileName).vecVertex_.end(), back_inserter(vecVertex_));

	// インデックス
	indexSize_ = StorageManager::Instance().GetObjModel(pFileName).indexSize_;
	pIndexBuffer_ = StorageManager::Instance().GetObjModel(pFileName).pIndexBuffer_;
	copy(StorageManager::Instance().GetObjModel(pFileName).vecIndex_.begin(), StorageManager::Instance().GetObjModel(pFileName).vecIndex_.end(), back_inserter(vecIndex_));

	// サブセット
	subsetSize_ = StorageManager::Instance().GetObjModel(pFileName).subsetSize_;
	copy(StorageManager::Instance().GetObjModel(pFileName).vecSubset_.begin(), StorageManager::Instance().GetObjModel(pFileName).vecSubset_.end(), back_inserter(vecSubset_));

	// マテリアル
	materialSize_ = StorageManager::Instance().GetObjModel(pFileName).materialSize_;
	copy(StorageManager::Instance().GetObjModel(pFileName).vecMaterial_.begin(), StorageManager::Instance().GetObjModel(pFileName).vecMaterial_.end(), back_inserter(vecMaterial_));

	// テクスチャ
	texPtrSize_ = StorageManager::Instance().GetObjModel(pFileName).texPtrSize_;
	copy(StorageManager::Instance().GetObjModel(pFileName).vecTexPtr_.begin(), StorageManager::Instance().GetObjModel(pFileName).vecTexPtr_.end(), back_inserter(vecTexPtr_));

	if(pVertexBuffer_ && pIndexBuffer_){ bStorage_ = true; }	// ストレージ使用フラグをオンに
	else{
		pVertexBuffer_ = nullptr;
		pIndexBuffer_ = nullptr;
		SetDebugMessage("ObjMeshLoadError! [%s] をストレージから読み込めませんでした。\n", pFileName);
		return false;
	}
...
bool ObjModel::LoadMaterialFromFile(const char * pFileName)
{
	...
	file.open(pFileName, ios::in);
	if(!file.is_open()){
		SetDebugMessage("ObjMaterialLoadError! [%s] をファイルから読み込めませんでした\n", pFileName);
		return false;
	}
...
bool ObjModel::LoadMaterialFromArchiveFile(const char* pArchiveFileName, const char * pFileName)
{
	...
	file.open(pFileName, ios::in);
	if(!file.is_open()){
		SetDebugMessage("ObjMaterialLoadError! [%s] を [%s] から読み込めませんでした。\n", pFileName, pArchiveFileName);
		return false;
	}
	tmpStream << file.rdbuf();
