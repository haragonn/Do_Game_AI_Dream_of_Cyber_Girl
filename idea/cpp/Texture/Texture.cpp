...
bool idea::Texture::LoadImageFromTextureView(ID3D11ShaderResourceView* textureView, unsigned int divX, unsigned int divY)
{
	if(pTextureView_){ return false; }	// 既に読み込み済みなら終了
	...
	// テクスチャービューから画像を読み込む
	pTextureView_ = textureView;

	if(pTextureView_){ bStorage_ = true; }	// ストレージ使用フラグをオンに
	else{
		pTextureView_ = nullptr;
		SetDebugMessage("TexturLoadError! テクスチャービューから読み込めませんでした。\n");
		return false;
	}

	strcpy_s(fileName_, 255, "FromTextureView");
	fileName_[255] = '\0';
