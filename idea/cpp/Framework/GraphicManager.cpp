...
namespace{
	...
	static const char* VS_FILENAME = "VSPera.cso";
	static const char* PS_PERA_DEF_FILENAME = "PSPeraDefault.cso";
	static const char* PS_PERA_FILENAME = "PSPera.cso";
	static const char* PS_PERA_SHADOW_FILENAME = "PSPeraShadow.cso";
}
...
GraphicManager::GraphicManager() :
	...
	height_(480),
	bWindowed_(true),
	blendState_(BLEND_STATE::BLEND_NONE),
	pD3DDevice_(nullptr),
...
bool GraphicManager::Init(HWND hWnd, UINT width, UINT height, bool bWindowed, UINT fps, bool bMSAA)
{
...
#if SHADER_FROM_ARCHIVE_FILE
	{
		...
		hr = pD3DDevice_->CreateVertexShader(&loader.GetData()[0], loader.GetSize(), NULL, &pPeraVertexShader_);
		if(FAILED(hr)){
			return false;
		}
		...
		// 入力レイアウト作成
		hr = pD3DDevice_->CreateInputLayout(layout, elem_num, &loader.GetData()[0], loader.GetSize(), &pPeraVertexLayout_);
	}
#else
	{
		// 頂点シェーダ作成
	...
	{
		ArchiveLoader loader;
		if(!loader.Load(ARCHIVE_FILENAME, PS_PERA_DEF_FILENAME)){
			return false;
		}

		hr = pD3DDevice_->CreatePixelShader(&loader.GetData()[0], loader.GetSize(), NULL, &pDefaultPixelShader_);
	...
	{
		ArchiveLoader loader;
		if(!loader.Load(ARCHIVE_FILENAME, PS_PERA_FILENAME)){
			return false;
		}

		hr = pD3DDevice_->CreatePixelShader(&loader.GetData()[0], loader.GetSize(), NULL, &pPeraPixelShader_);
	...
	{
		ArchiveLoader loader;
		if(!loader.Load(ARCHIVE_FILENAME, PS_PERA_SHADOW_FILENAME)){
			return false;
		}

		hr = pD3DDevice_->CreatePixelShader(&loader.GetData()[0], loader.GetSize(), NULL, &pPeraShadowPixelShader_);
...
#else
	{
		BYTE* data;
		int size = 0;
		size = ReadShader(PS_PERA_DEF_FILENAME, &data);
		if(size == 0){
			return false;
		}
	...
	}
	{
		BYTE* data;
		int size = 0;
		size = ReadShader(PS_PERA_FILENAME, &data);
		if(size == 0){
			return false;
		}
	...
	}
	{
		BYTE* data;
		int size = 0;
		size = ReadShader(PS_PERA_SHADOW_FILENAME, &data);
		if(size == 0){
			return false;
		}
	...
	// ブレンディングステート生成
	if(!SetBlendState(BLEND_STATE::BLEND_ALIGNMENT)){
		return false;
	}
...
{
	if(blendState == BLEND_STATE::BLEND_NONE || blendState >= BLEND_STATE::BLEND_MAX){
		return false;
	}	// 無効なステータス
	...
	switch(blendState){
		case BLEND_STATE::BLEND_DEFAULT:	// アルファブレンドなし
		...
		case BLEND_STATE::BLEND_ALIGNMENT:	// アルファブレンドあり
		...
		case BLEND_STATE::BLEND_ADD:			// 加算合成
			renderTarget.BlendEnable = TRUE;
			renderTarget.SrcBlend = D3D11_BLEND_SRC_ALPHA;
		...
		case BLEND_STATE::BLEND_SUBTRACT:	// 減算合成
			renderTarget.BlendEnable = TRUE;
			renderTarget.SrcBlend = D3D11_BLEND_SRC_ALPHA;
			renderTarget.DestBlend = D3D11_BLEND_ONE;
		...
		case BLEND_STATE::BLEND_MULTIPLE:	// 乗算合成
			renderTarget.BlendEnable = TRUE;
...
bool GraphicManager::DrawMask(bool bVisible)
{
	...
	if(FAILED(pD3DDevice_->CreateDepthStencilState(&ddsDesc, &pDepthStencilState))){
		return false;
	}
...
bool GraphicManager::ChangeDisplayMode(bool bWindowed)
{
	...
	if(FAILED(pSwapChain_->ResizeBuffers(4, width_, height_, DXGI_FORMAT_R8G8B8A8_UNORM, 0))){
		return false;
	}
